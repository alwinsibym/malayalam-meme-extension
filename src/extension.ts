...
import { MemeManager, MemeCategory, MemeStats, COMPANION_DIALOGUES } from './memeManager';
import { ControlPanelWebview } from './controlPanelWebview';
import * as path from 'path';
import * as fs from 'fs';

let idleTimer: NodeJS.Timeout | undefined;
let wpmTimer: NodeJS.Timeout | undefined;
let charCount = 0;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('malayalamMemes.sidebar', sidebarProvider)
    );
    ...
    if (vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
        memeManager.play(MemeCategory.FolderOpen);
    } else {
        memeManager.play(MemeCategory.Startup);
    }

    context.subscriptions.push(vscode.workspace.onDidChangeWorkspaceFolders((e) => {
        if (e.added.length > 0) {
            memeManager.play(MemeCategory.FolderOpen, true);
        }
    }));
    ...
    if (vscode.window.onDidEndTerminalShellExecution) {
        context.subscriptions.push(vscode.window.onDidEndTerminalShellExecution(e => {
            if (e.exitCode === 0) memeManager.play(MemeCategory.Success);
            else if (e.exitCode !== undefined) memeManager.play(MemeCategory.TerminalError);
        }));
    }
    ...
    context.subscriptions.push(vscode.tasks.onDidEndTaskProcess((e) => {
        if (e.execution.task.name.toLowerCase().includes('push')) {
            memeManager.play(MemeCategory.GitPush);
        }
    }));
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((doc) => {
        ...
        if (errorCount > 0) {
            memeManager.play(MemeCategory.ProjectError);
        }
    }));
...
function registerAllCommands(context: vscode.ExtensionContext, memeManager: MemeManager) {
    // 1. Control Panel
    context.subscriptions.push(vscode.commands.registerCommand('malayalamMemes.controlPanel', () => {
        ControlPanelWebview.createOrShow(context.extensionUri, memeManager);
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('malayalamMemes.testTerminalError', () => {
        memeManager.play(MemeCategory.TerminalError, true);
    }));
...
function updateVolumeBar() {
    const vol = vscode.workspace.getConfiguration('malayalamMemes').get<number>('volume') || 0.5;
    volumeItem.text = `$(unmute) ${Math.round(vol * 100)}%`;
}

function setupInactivityTimer(context: vscode.ExtensionContext) {
    if (idleTimer) clearInterval(idleTimer);
    const timeout = (vscode.workspace.getConfiguration('malayalamMemes').get<number>('inactivityTimeout') || 5) * 60000;
    idleTimer = setInterval(() => MemeManager.getInstance(context).play(MemeCategory.Inactivity), timeout);
}

function resetInactivityTimer(context: vscode.ExtensionContext) { setupInactivityTimer(context); }

// ══════════════════════════════════════════════════════════════════════
//  SIDEBAR PROVIDER
// ══════════════════════════════════════════════════════════════════════
class MemeSidebarProvider implements vscode.WebviewViewProvider {
    private _view?: vscode.WebviewView;
    constructor(private readonly _extensionUri: vscode.Uri, private readonly _memeManager: MemeManager) {
    ...
    public resolveWebviewView(webviewView: vscode.WebviewView) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            ...
            if (msg.command === 'petClick' || msg.command === 'autoTalk') {
                const dialogues = COMPANION_DIALOGUES[companion] || [];
...
export function deactivate(): Thenable<void> | undefined {
    ...
    const memeManager = MemeManager.getInstance(undefined as any);
    if (memeManager) {
