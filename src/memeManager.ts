...
export class MemeManager {
    ...
    public async play(category: MemeCategory, force: boolean = false) {
        ...
        const mode = config.get<string>('intensityMode') || 'Balanced';
        ...
        if (mode === 'Aggressive') {
            ...
            if (extraSpecial.length > 0) pool.push(...extraSpecial);
        }
